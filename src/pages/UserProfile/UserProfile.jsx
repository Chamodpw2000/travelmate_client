...
const UserProfile = () => {
    ...
    const fetchUser = async () => {
      try {
        ...
        const res = await axios.get(
          "https://travelmate-backend-zuqb.onrender.com/user/get-user-from-email",
          {
            params: { email },
          }
        );
  ...
  const getAvatarUrl = () => {
    ...
    if (user) {
      ...
      return `https://ui-avatars.com/api/?name=${encodeURIComponent(
        name
      )}&background=dde8ee&color=white&size=200`;
    }
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      await axios.put(
        "https://travelmate-backend-zuqb.onrender.com/user/update-user",
        updatedUser
      );
